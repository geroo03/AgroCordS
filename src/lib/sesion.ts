 ...
 *
 * Todavía no hay backend de autenticación conectado desde el navegador (ver
 * `app/login/page.tsx` y `server/` — el backend Express con Supabase Auth ya
 * existe, pero esta pantalla no lo llama todavía): `iniciarSesion` se llama
 * recién cuando el flujo de magic link "confirma" el ingreso, como una
 * SIMULACIÓN explícita para la demo — no valida ningún token real. En
 * producción esto vendría de una sesión validada en el servidor, pero la
 * interfaz — `obtenerSesion()` / `cerrarSesion()` — no cambiaría: mismo
 * aislamiento que `plan.ts` usa para poder reemplazar localStorage por
