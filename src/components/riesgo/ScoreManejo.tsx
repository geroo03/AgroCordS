...
export default function ScoreManejo({ resultado }: { resultado: TScoreManejo }) {
  if (!resultado.confiable) {
    return (
      <div className="shadow-sunken rounded-xl bg-[#f2f3ff] p-4 text-[16px] text-tinta/80">
  ...
  return (
    <section>
      <p
        className={`text-[clamp(46px,13vw,64px)] font-extrabold leading-none tabular-nums ${COLOR_BANDA[resultado.banda]}`}
      >
        {resultado.score}
      </p>
      <p className={`mt-1 text-[24px] font-bold ${COLOR_BANDA[resultado.banda]}`}>
        {ETIQUETA_BANDA[resultado.banda]}
      </p>

      <ul className="mt-4 space-y-2">
        {resultado.factores.map((f) => (
          <li key={f.etiqueta} className="shadow-extruded rounded-lg bg-white p-3">
            <p className="flex items-center justify-between gap-2 text-[16px] font-semibold">
            ...
            <p className="mt-1 text-[16px] text-tinta/70">{f.detalle}</p>
          </li>
        ))}
      </ul>
    </section>
