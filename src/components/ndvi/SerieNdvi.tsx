...
export default function SerieNdvi({ serie, seleccionada, onSeleccionar }: Props) {
  ...
  return (
    <section>
      <h2 className="text-[24px] font-bold">Observaciones disponibles</h2>
      ...
      <div className="mt-1 flex gap-4 text-[14px] text-tinta/70">
        ...
        {serie.map((l) => {
          ...
          return (
            <button
              ...
              aria-label={etiqueta}
              className={`min-h-11 shrink-0 rounded-lg px-3 text-[16px] font-semibold tabular-nums transition-all active:scale-95 ${
                l.fecha === seleccionada
                  ? "shadow-sunken bg-pizarra text-white"
                  : sinDato
                    ? "shadow-extruded-sm border border-dashed border-[#d8e2ef] bg-white text-tinta/45"
                    : "shadow-extruded-sm bg-white text-tinta"
              }`}
            >
              {fechaCorta(l.fecha)}
            </button>
