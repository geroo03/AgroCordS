  ...
  return (
    <div className="mt-3 rounded-xl border border-niebla p-4">
      ...
      {fase === "verificado" && comprobante ? (
        <div className="mt-3 rounded-lg bg-optima/10 p-3 text-[16px]">
        ...
        <>
            ...
            <div className="mt-2 flex gap-2">
              {tokensDisponibles().map((s) => (
                <button
                  ...
                  className={`min-h-11 rounded-lg border px-3 text-[16px] font-semibold ${
          ...
          {hash && fase !== "verificado" ? (
            <p className="mt-2 truncate text-[14px] text-tinta/60">Transacción: {hash}</p>
          ) : null}

          {error ? (
            <div className="mt-2 flex items-center justify-between gap-2 text-[16px] text-bloqueo">
              <span>{error}</span>
              {hash ? (
                <button
                  type="button"
